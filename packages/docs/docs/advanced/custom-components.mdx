...
[Circle](/api/2d/component/Rect) that can abstract rendering and data
...
[Node](/api/2d/components/Node) or one of its subclasses. If you dont want to
...
Every component _requires_ a constructor. The format of the constructor will
always look similar to this, with optional argument for the properties:
...
The `props` parameter can also be useful outside of the `super()` call to access
