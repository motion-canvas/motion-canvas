...
3. Start the editor:
   ...
   npm run serve
   ```
   You may see a yellow error message in the terminal saying "Could not
   auto-determine entry point", but it's not important, and you can safely
   ignore it. The editor can be accessed by visiting
