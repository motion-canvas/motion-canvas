...
a `stacktrace` or an `object`. That's where
[`LogPayloads`](/api/core/index/LogPayload) come in. These can be used to
provide more information to your log messages.
...
with the [`Logger`](/api/core/index/Logger):
