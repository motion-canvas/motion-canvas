...
export default makeScene2D(function* (view) {
  ...
  //highlight-start
  yield* tween(2, value => {
    circle().position.x(map(-150, 150, value));
  });
  //highlight-end
});
```
...
yield *
  tween(2, value => {
    circle().position.x(map(-150, 150, easeInOutCubic(value)));
  });
```
...
```tsx editor mode=code ratio=16/9
import {
  Circle,
  Grid,
  Rect,
...
export default makeScene2D(function* (view) {
  ...
  view.add(
    <Node y={-15}>
      ...
      <Grid size={350} stroke={'#444'} lineWidth={1.5} spacing={50}>
        <Rect
          layout
          size={50}
          offset={[-1, 1]}
          ...
          y={() => value() * -250 + 150}
        >
          <Circle size={30} fill={'#C22929'} margin={10}></Circle>
        </Rect>
      </Grid>
      ...
      <Node position={[-200, -200]}>
        ...
        <Layout y={() => value() * -250 + 325}>
          <Txt
            ...
            y={1.5}
          ></Txt>
          <Circle size={15} fill={'#DDD'}></Circle>
        </Layout>
...
export default makeScene2D(function* (view) {
  ...
  yield* spring(PlopSpring, -200, 200, 1, value => {
    circle().position.x(value);
  });
  yield* spring(SmoothSpring, 200, -200, value => {
    circle().position.x(value);
  });
});
```
...
export default makeScene2D(function* (view) {
  ...
  circle().save();
  yield* all(circle().position.x(150, 1), circle().scale(1, 1));
