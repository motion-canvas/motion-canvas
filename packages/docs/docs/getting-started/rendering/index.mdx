...
Clicking the `RENDER` button will initialize rendering (The button will change
to `RENDERING` for the duration of the process). Motion Canvas will play through
...
configuration file. You can use the `OUTPUT DIRECTORY` button to reveal the
...
You can also abort the process at any time by pressing the `RENDERING` button.
