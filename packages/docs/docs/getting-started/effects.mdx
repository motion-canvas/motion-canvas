...
Effects are created using either
[`createEffect()`](/api/core/signals#createEffect) or
[`createDeferredEffect()`](/api/core/signals#createDeferredEffect) functions.
The first argument specifies the callback to invoke:
...
This time the `area` signal is invoked inside an effect, instead of being called
directly. Now whenever the `radius` signal changes, the effect is triggered,
which recalculates the `area` signal instantly.
...
calculate, this may affect the seeking performance.
...
available after the current generator step is finished, but before the rendering
starts.
...
the node hierarchy so that the amount of circles displayed matches the value of
the `count` signal:
