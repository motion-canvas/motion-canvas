...
For this specific use case there is however another helper, `word`. It expects 3
parameters - the starting line and character offset, aswell as a word length.
...
`Infinity`, or simply pass `DEFAULT` from as a selection:
