...
automatically highlight many common languages. That code can then be modified
in place to present changes to the viewer. Modified code will animate from the
prior code to the new code by removing old code, translating the remaining
code, and inserting new code. You may also "select" code in order to call
attention to important snippets.
...
for selection. Though they all return a two by two array of numbers, they can
be more descriptive than an array literal.

The first and most commonly used  is the `range` function. `range(0, 4, 0, 10)` is
equivalent to `[[0, 4], [0, 10]]`, making `range` a direct substitute for an
array literal.
