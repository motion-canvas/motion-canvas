...
signal created using
[`Code.createSignal()`](/api/2d/components/Code#createSignal). While the generic
[`createSignal()`](/api/core/signals#createSignal) would work fine in these
simple examples, the specialized signal will shine once we start animating our
code snippets.
