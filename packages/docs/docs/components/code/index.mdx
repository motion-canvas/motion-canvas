...
[`Lezer`](https://lezer.codemirror.net/) to parse and highlighting the code but
to do that it needs the grammar for the language you're using. We can set that
up in our project configuration file.
...
we'll install the `javascript` grammar:

```bash
npm i @lezer/javascript
```

Then, in your project configuration, import the grammar and register it via
`LezerHighlighter`:
...
This will set up `@lezer/javascript` as the default parser for all your code
snippets. If you want to use more than one language, check out the
...
- The backslash character (`\`) at the very beginning is used to escape the
  first newline character. This lets us start the snippet on a new line without
  actually including an empty line at the start of the snippet. Without the
  slash, the equivalent code would have to be written as:
  ```tsx
  view.add(
    <Code
      fontSize={28}
...
Template strings allow us to easily include variables in our code snippets with
`${}` syntax. In the example below, `${name}` will be replaced with the value of
the `name` variable (which is `number` in this case):
...
Any valid JavaScript expression can be used inside the `${}` syntax and the
result will be included in the code snippet:
...
work as expected. Invoking a signal inside a template string will use its
current value and then never update the snippet again, even if the signal
changes:
...
export default makeScene2D(function* (view) {
  ...
  yield* waitFor(1);
  nameSignal('newValue');
  // The code snippet will still display "number" instead of "newValue".
  yield* waitFor(1);
});
```
...
executed. To work around this, we provide a custom [tag function][tag-function]
...
You might have noticed that in these examples we used a specialized type of
signal created using [`Code.createSignal()`](/api/2d/code/Code#createSignal).
While the generic [`createSignal()`](/api/core/signals#createSignal) would work
fine in these simple examples, the specialized signal will shine once we start
animating our code snippets.
...
snippet, you can use the [`append`](/api/2d/code/CodeSignal#append) and
[`prepend`](/api/2d/code/CodeSignal#prepend) methods. They can either modify the
code immediately or animate the changes over time:
...
[`insert`](/api/2d/code/CodeSignal#insert),
[`replace`](/api/2d/code/CodeSignal#replace), and
[`remove`](/api/2d/code/CodeSignal#remove) to modify the code at specific
...
The [`edit`](/api/2d/code/CodeSignal#edit) method offers a different way of
defining code transitions. It's used together with the
...
Notice that all the methods we used above are not invoked on the `Code` node but
...
the ones we create using
[`Code.createSignal()`](/api/2d/code/Code#createSignal). This means that all the
animation methods are also available on your own signals:
...
The [`selection`](/api/2d/code/CodeSignal#selection) property can be used to
...
[`getPointBBox`](/api/2d/code/Code#getPointBBox) and
[`getSelectionBBox`](/api/2d/code/Code#getSelectionBBox) can be used to retrieve
the position and size of a specific character or a range of characters,
respectively. The returned value is a [`bounding box`](/api/core/types/BBox) in
the local space of the `Code` node.

In the following example, we use `getSelectionBbox` to draw a rectangle around
the word `log`:
...
function hello() {
  ...
  const range = createSignal(() => {
    ...
    // one for each line in the range. We'll just use
    // the first one for this example.
...
You can register parsers for multiple different languages by giving them a
unique name during registration:

```ts
import {LezerHighlighter} from '@motion-canvas/2d';
import {parser as jsParser} from '@lezer/javascript';
// highlight-next-line
import {parser as rsParser} from '@lezer/rust';

LezerHighlighter.registerParser(jsParser);
// highlight-next-line
LezerHighlighter.registerParser(rsParser, 'rs');
```

This name can then be passed to the [`dialect`](/api/2d/components/Code#dialect)
property of a specific `Code` node to use a different parser:

```tsx editor mode=code
import {makeScene2D, Code} from '@motion-canvas/2d';

export default makeScene2D(function* (view) {
  // Font properties configured for the view are
  // inherited by all text nodes, including Code:
  view.fontSize(28).fontFamily('JetBrains Mono');

  view.add(
    <>
      <Code
        // this node uses the default parser (javascript)
        offsetX={-1}
        x={-400}
        code={`
function hello() {
...
`}
      />
      <Code
        // this node uses the "rs" parser (rust)
        dialect={'rs'}
        offsetX={1}
        x={400}
        code={`
