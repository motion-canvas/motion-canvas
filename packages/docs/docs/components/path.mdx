...
The `Path` allows us to draw and animate SVG paths.

:::tip

Path nodes are great for displaying pre-calculated SVG paths, but the format can be confusing. If you want to have more
intuitive control of the path, and control each point within Motion Canvas, we reccomend checking out the [`Spline`][spline] component.

:::

## Defining paths

We can specify the path to be displayed by setting the data property of the Path component:
...
Similar to the [`Spline`][spline] and [`Bézier curve`][bezier] components, the `Path` component has
a `getPointAtPercentage` method that can be used to animate an object along it.
